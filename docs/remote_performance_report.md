...
## 2026-06-03 POSIX Stream Zero-Copy Send Fallback

This pass makes the public stream zero-copy send helpers usable on the
supported POSIX backend set. Linux keeps the existing io_uring zero-copy submit
path; non-Linux now falls back to ordinary POSIX socket sends instead of
returning `ENOSYS`.

Changes under validation:

- Changed `af::io_send_zc_some` to use `::send` as the non-Linux fallback while
  keeping the Linux io_uring zero-copy submit path and readiness fallback.
- Changed `af::io_sendv_zc_some` to use `::sendmsg` as the non-Linux fallback
  while keeping the Linux io_uring sendmsg-zc submit path and readiness
  fallback.
- Moved POSIX stream test helpers and basic/vectored/zero-copy-send task
  support out of the Linux-only test include block.
- Changed stream adapter tests to use portable socketpair setup and to run
  basic stream, vectored stream, `send_zc`, and `sendv_zc` paths through the
  native IO backend on macOS/BSD instead of skipping.

Correctness checks:

- Local macOS Debug built `asyncflow_runtime_tests`.
- Local macOS Debug ctest
  `StreamAdapter.*SocketBytes|SendZc|SendvZc|ZeroCopy`: 7 tests selected, 0
  failures; the native kqueue stream basic/vectored/send_zc/sendv_zc tests
  passed, while Linux-only epoll/io_uring-specific tests were skipped by
  capability logic.
- Remote GCC Debug,
  `ghcr.io/hhhflow2020/cpp-dev-gcc:bookworm-v2.0.3`,
  `seccomp=unconfined`: clean-first rebuilt `asyncflow_runtime_tests`; ctest
  `StreamAdapter.*SocketBytes|SendZc|SendvZc|ZeroCopy|UringIoRuntimeSocketStreamFixture.IoUringThreadSendZc`
  reported 7 tests, 0 failures.
- Remote Clang Debug,
  `ghcr.io/hhhflow2020/cpp-dev-clang:bookworm-v2.0.3`,
  `seccomp=unconfined`: clean-first rebuilt `asyncflow_runtime_tests`; the same
  ctest selection reported 7 tests, 0 failures.

Interpretation:

- This improves the public POSIX IO API surface: users can call stream
...
## 2026-06-03 IO Wait Cancel Entry Split

This pass moves the public executor IO wait/cancel control entry bodies out of
the core executor declaration header and into the generic IO backend
implementation header. The core executor now keeps declarations for these
entries, while the backend header owns the readiness/completion cancellation
dispatch that already depends on backend-specific helpers.

Changes under validation:

- Moved `register_io_wait()` to `runtime_executor_io_backend.hpp`.
- Moved Linux `cancel_io_completion()` to
  `runtime_executor_io_backend.hpp`.
- Moved `cancel_io()` to `runtime_executor_io_backend.hpp`.
- `runtime_executor.hpp` dropped from 1877 lines to 1793 lines. The moved
  functions remain template-inline definitions included after the executor
  class, so this does not add dispatch, allocation, locking, atomics, or
...
- Remote Clang Debug,
  `ghcr.io/hhhflow2020/cpp-dev-clang:bookworm-v2.0.3`,
  `seccomp=unconfined`: rebuilt the same targets; the same ctest selection
  reported 51 tests selected, 0 failures, with the same capability skip. A
  separate `Uring|IoUring` ctest selection reported 43 tests selected, 0
  failures, with the same 3 platform/capability skips. The same four example
  binaries also ran successfully.

Interpretation:

- This is a modularity-only runtime cleanup. It keeps IO wait/cancel dispatch
...
## 2026-06-03 io_uring Submission Queue Split

This pass moves Linux io_uring submission queue bodies out of the generic IO
backend implementation header and into a dedicated submission header. The
generic backend file now keeps cross-platform IO entry points, shared native
wait helpers, and CQ/operation completion logic, while
`runtime_executor_io_uring_submission.hpp` owns SQE reservation and submission
flush behavior.

Changes under validation:

- Added `runtime_executor_io_uring_submission.hpp`.
- Moved `reserve_io_uring_sqe()`, `flush_io_uring_submissions()`,
  `flush_io_uring_submissions_or_fail()`, and `submit_io_uring_cancel()` to the
  new header.
- `runtime_executor_io_backend.hpp` dropped from 717 lines to 608 lines. The
  new submission header is 112 lines. The moved functions remain
  template-inline definitions included after the executor class, so this does
  not add dispatch, allocation, locking, atomics, or cross-thread
  communication.
...
- This is a modularity-only runtime cleanup. It keeps io_uring SQ ownership,
  pending-submission accounting, syscall retry behavior, wake/cancel semantics,
  locks, atomics, memory ordering, and cache layout unchanged.
