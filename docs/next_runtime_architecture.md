...
- 当前已提供 `af::runtime` 实例生命周期外壳：构造时使用结构化配置解析校验，`start()` 按配置启动 runtime 线程，空闲线程使用 atomic/futex wait，`stop()` 唤醒并 join；后续还需要把 task inbox、task pool、timer、reactor 和 logger 接入该实例 runtime。
