...
- The native readiness example now keeps POSIX socketpair setup, fd flags, and one-byte peer write helpers in a focused support header. The executable body uses `af::platform_posix` and framework IO flow directly instead of carrying direct platform system headers or `_WIN32` branches.
