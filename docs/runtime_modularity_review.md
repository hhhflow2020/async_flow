...
- The largest remaining examples/tests are now fixture/state-machine files, not the runtime shell. `runtime_io_uring_socket_datagram_tests.cpp`, `runtime_io_stream_transfer_tests.cpp`, `io_rpc_length_prefixed_server.hpp`, and `io_uring_fixed_file_task.hpp` have been reduced by moving repeated setup or role-specific tasks into support fragments; the remaining notable example candidate is `io_uring_file_lifecycle_task.hpp`.
...
- P2: several IO tests and examples remain moderately dense after the first pass. `runtime_io_uring_socket_datagram_tests.cpp` and `runtime_io_stream_transfer_tests.cpp` were reduced by extracting repeated fixture setup into shared test support; `io_rpc_length_prefixed_server.hpp` and `io_uring_fixed_file_task.hpp` were reduced to small task/umbrella shells over role or operation-family fragments. The notable remaining example file is `io_uring_file_lifecycle_task.hpp`. Future edits should split by protocol role, transfer mode, or operation family instead of appending new states to the existing file.
...
- P2: several examples remain long because they combine protocol framing, socket IO, task state machines, and result reporting. The fixed-file round trip and length-prefixed RPC server paths have been reduced to focused fragments; the biggest current examples are now the length-prefixed RPC client, file lifecycle, and UDP recv/recvmsg multishot examples. Prefer protocol/helper headers plus small task headers for future edits.
