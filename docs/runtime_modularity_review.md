...
- `include/af/async_runtime.hpp` is now a normal runtime class shell with public aliases/API declarations, static runtime state, and no class-body fragment includes. Namespace-scope inline implementation headers are included after the class definition, and platform runtime system headers are centralized in `runtime_platform_headers.hpp`.
...
- The IO timeout example is no longer Linux-only. It now owns a focused native
...
- Stream zero-copy send helpers now preserve the Linux io_uring zero-copy path
...
- Datagram receive and readiness tests now run against the native IO backend on
  macOS/BSD as well as Linux. The coverage now exercises UDP recv/recvv,
...
- `RuntimePublicIo` now centralizes executor lookup and Linux-only submit
...
- Linux epoll wake draining and poll/native event mask conversion helpers now
  ...
  epoll backend.
- Native IO wait entry/registration helper bodies now live in
...
- Linux io_uring submission queue bodies now live in
  `runtime_executor_io_uring_submission.hpp` instead of the generic IO backend
  file. The new header owns SQE reservation, submission flush, flush-or-fail,
  and async-cancel SQE submission while preserving the same inline template
  visibility and ring memory-ordering operations.
...
- The TCP echo server shutdown notifier now uses the same portable `pipe` +
  `fcntl` path on all POSIX platforms. Its support state header no longer needs
  a Linux-only `pipe2` branch.
- POSIX example socket flag handling is now centralized in
  `examples/support/posix_socket_flags.hpp`. TCP echo, TCP connect/accept, RPC
...
- Linux-only epoll/io_uring/sendfile example support now also uses the shared
  POSIX socket helper for synchronous `socket` and `socketpair` setup. Async
  `io_socket` tasks use the same helper to compute creation-time flags instead
  of spelling out `SOCK_NONBLOCK | SOCK_CLOEXEC` at each call site.
...
- `include/af/io_common.hpp` is now a small umbrella over focused common headers: basic socket/error helpers, wait-state helpers, fixed-file vectored helpers, platform-specific eventfd/timerfd helpers, and deadline state. The active internal headers use normal module names (`io_common_base.hpp`, `io_common_state.hpp`, `io_common_fixed_file.hpp`, `io_common_event_timer.hpp`, `io_common_deadline.hpp`) instead of `*_fragment.hpp`.
...
- `include/af/io_types.hpp` is now a small public umbrella over platform IO includes/types, base IO typedefs/views, Linux provided-buffer rings, `IoStatus`, and `UniqueFd`. The active internal headers use normal module names (`io_types_platform.hpp`, `io_types_base.hpp`, `io_types_provided_buffer.hpp`, `io_types_status.hpp`, `io_types_unique_fd.hpp`) instead of `*_fragment.hpp`; Linux-only ring storage and POSIX fd ownership stay inline.
...
- `include/af/signal.hpp` is now a small public shell over `detail/signal/signal_platform.hpp`; POSIX signal masks, timed waits, fallback polling, and signal-ignore implementation are no longer embedded in the public class body, and the signal detail implementation no longer carries Windows fallback branches.
