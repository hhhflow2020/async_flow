...
[[nodiscard]] ShardedOps<CrudOp<Key, Value>> split_crud_ops(
    std::vector<CrudOp<Key, Value>>&& ops,
...
template <typename Key, typename Value, typename ShardFn>
[[nodiscard]] ShardedOps<CrudOp<Key, Value>> split_change_batch(
    ChangeBatch<Key, Value>& batch,
    std::uint16_t shard_count,
    ShardFn&& shard_fn) {
