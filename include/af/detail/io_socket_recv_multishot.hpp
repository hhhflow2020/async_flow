...
#error "io_socket_recv_multishot.hpp is internal to af/io_socket.hpp"
#endif

#if defined(__linux__)
template <typename TaskT>
[[nodiscard]] IoStatus
io_recv_multishot(TaskT &task, typename TaskT::Thread thread, int fd,
  ...
  *buffer_id = 0;
  ...
  if (TaskT::Runtime::io_submit_recv_multishot(thread, fd, buffer_group, flags,
                                               &task, &state.wait)) {
    state.waiting = true;
