...
template <typename TraitsT> struct RuntimeConfig {
  ...
  static_assert(TraitsT::thread_count > 0,
                "AsyncRuntime requires at least one fixed thread");
  ...
  static constexpr unsigned io_uring_cq_entries =
      TraitConfig::io_uring_cq_entries;
  static constexpr unsigned io_uring_setup_flags =
      TraitConfig::io_uring_setup_flags;
  static constexpr bool io_uring_setup_sqpoll =
      TraitConfig::io_uring_setup_sqpoll;
  ...
  static constexpr bool io_uring_setup_submit_all =
      TraitConfig::io_uring_setup_submit_all;
  ...
  static_assert(spsc_queue_capacity > 0,
  ...
  static_assert(io_uring_cq_entries == 0U ||
                    std::has_single_bit(io_uring_cq_entries),
