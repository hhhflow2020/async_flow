...
private:
    ...
    struct LocalCache {
        ...
        void reset_for(ObjectPoolCore *pool) noexcept {
            if (owner == pool) {
                return;
            }
            flush();
        ...
        void discard_if_owner(const ObjectPoolCore *pool) noexcept {
            if (owner == pool) {
                size = 0;
    ...
    struct DirectReleaseSet {
        ObjectPoolCore *owners[direct_release_set_size]{};
        std::size_t next_victim{0};

        [[nodiscard]] bool contains(ObjectPoolCore *pool) const noexcept {
            if (owners[0] == pool) {
                return true;
            }
        ...
        void insert(ObjectPoolCore *pool) noexcept {
            if (contains(pool)) {
                return;
            }
        ...
        void erase(const ObjectPoolCore *pool) noexcept {
            for (ObjectPoolCore *&owner : owners) {
                if (owner == pool) {
                    owner = nullptr;
                }
            }
    ...
    struct SingleLocalCacheSet {
        ...
        [[nodiscard]] LocalCache &get(ObjectPoolCore *pool) noexcept {
        ...
        [[nodiscard]] LocalCache *find_release_cache(ObjectPoolCore *pool) noexcept {
            if (primary.owner == pool) [[likely]] {
                return &primary;
            }
        ...
        [[nodiscard]] AF_DETAIL_NOINLINE LocalCache &get_slow(ObjectPoolCore *pool) noexcept {
            direct_release_erase(pool);
            primary.reset_for(pool);
            return primary;
        }

        [[nodiscard]] static bool direct_release_contains(ObjectPoolCore *pool) noexcept {
        ...
        static void direct_release_insert(ObjectPoolCore *pool) noexcept {
            if constexpr (remote_release_batch_size == 1U) {
        ...
        static void direct_release_erase(const ObjectPoolCore *pool) noexcept {
            if constexpr (remote_release_batch_size == 1U) {
    ...
    struct MultiLocalCacheSet {
        ...
        find_release_cache_slow(ObjectPoolCore *pool) noexcept {
            for (std::size_t i = 0; i < overflow_cache_count; ++i) {
                if (overflow[i].owner == pool) {
                    set_active_overflow(i);
                    return &overflow[i];
        ...
        [[nodiscard]] LocalCache *next_overflow_hint(ObjectPoolCore *pool) noexcept {
