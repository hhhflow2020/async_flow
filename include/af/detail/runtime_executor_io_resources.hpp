...
[[nodiscard]] bool
Executor<RuntimeT, TraitsT>::io_backend_available() const noexcept {
  return native_io_backend_available();
}

template <typename RuntimeT, typename TraitsT>
[[nodiscard]] bool
Executor<RuntimeT, TraitsT>::io_uring_backend_available() const noexcept {
...
template <typename RuntimeT, typename TraitsT>
[[nodiscard]] int
Executor<RuntimeT, TraitsT>::io_uring_backend_error() const noexcept {
#if defined(__linux__)
  return io_uring_fd_ >= 0
             ? 0
...
template <typename RuntimeT, typename TraitsT>
[[nodiscard]] bool
Executor<RuntimeT, TraitsT>::io_uring_poll_available() const noexcept {
#if defined(__linux__)
...
#if !defined(_WIN32)
template <typename RuntimeT, typename TraitsT>
[[nodiscard]] bool Executor<RuntimeT, TraitsT>::register_io_uring_buffers(
    const iovec *buffers, unsigned buffer_count, int *error) noexcept {
...
template <typename RuntimeT, typename TraitsT>
[[nodiscard]] bool
Executor<RuntimeT, TraitsT>::unregister_io_uring_buffers(int *error) noexcept {
  AF_ASSERT(RuntimeT::current_thread_index_ == index_ &&
...
template <typename RuntimeT, typename TraitsT>
[[nodiscard]] bool
Executor<RuntimeT, TraitsT>::register_io_uring_provided_buffer_ring(
    void *ring, unsigned ring_entries, std::uint16_t buffer_group,
    int *error) noexcept {
  AF_ASSERT(
...
template <typename RuntimeT, typename TraitsT>
[[nodiscard]] bool
Executor<RuntimeT, TraitsT>::unregister_io_uring_provided_buffer_ring(
    std::uint16_t buffer_group, int *error) noexcept {
  AF_ASSERT(RuntimeT::current_thread_index_ == index_ &&
...
template <typename RuntimeT, typename TraitsT>
[[nodiscard]] bool Executor<RuntimeT, TraitsT>::register_io_uring_files(
    const int *files, unsigned file_count, int *error) noexcept {
  AF_ASSERT(RuntimeT::current_thread_index_ == index_ &&
  ...
  if (detail::sys_io_uring_register(io_uring_fd_, IORING_REGISTER_FILES, files,
                                    file_count) != 0) {
    if (error != nullptr) {
      *error = errno == 0 ? EIO : errno;
    }
...
template <typename RuntimeT, typename TraitsT>
[[nodiscard]] bool
Executor<RuntimeT, TraitsT>::unregister_io_uring_files(int *error) noexcept {
  AF_ASSERT(RuntimeT::current_thread_index_ == index_ &&
...
template <typename RuntimeT, typename TraitsT>
[[nodiscard]] bool Executor<RuntimeT, TraitsT>::update_io_uring_files(
    unsigned offset, const int *files, unsigned file_count,
    int *error) noexcept {
  AF_ASSERT(RuntimeT::current_thread_index_ == index_ &&
