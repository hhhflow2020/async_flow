...
public:
    ...
    [[nodiscard]] IoStatus openat2(TaskT &task, const char *path, const struct open_how *how,
                                   int *opened_fd, IoOpState &state) const noexcept {
    ...
    template <typename TaskT>
    [[nodiscard]] IoStatus mkdirat(TaskT &task, const char *path, std::uint32_t mode,
    ...
    template <typename TaskT>
    [[nodiscard]] IoStatus symlinkat(TaskT &task, const char *target, const char *link_path,
                                     IoOpState &state) const noexcept {
        static_assert(std::is_same_v<typename TaskT::Thread, ThreadT>,
                      "IoDirectory thread type must match the task runtime thread type");
    ...
    template <typename TaskT>
    [[nodiscard]] IoStatus linkat(TaskT &task, int old_dir_fd, const char *old_path,
