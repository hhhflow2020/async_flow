...
public:
  ...
  constexpr IoDirectory(ThreadT thread, int fd) noexcept
  ...
  [[nodiscard]] ThreadT thread() const noexcept { return thread_; }

  template <typename TaskT>
  [[nodiscard]] IoStatus openat2(TaskT &task, const char *path,
  ...
  template <typename TaskT>
  [[nodiscard]] IoStatus mkdirat(TaskT &task, const char *path,
                                 std::uint32_t mode,
                                 IoOpState &state) const noexcept {
    static_assert(
        std::is_same_v<typename TaskT::Thread, ThreadT>,
  ...
  template <typename TaskT>
  [[nodiscard]] IoStatus symlinkat(TaskT &task, const char *target,
  ...
  template <typename TaskT>
  [[nodiscard]] IoStatus linkat(TaskT &task, int old_dir_fd,
    ...
    static_assert(
        std::is_same_v<typename TaskT::Thread, ThreadT>,
        "IoDirectory thread type must match the task runtime thread type");
    return af::io_linkat(task, thread_, old_dir_fd, old_path, fd_, new_path,
                         flags, state);
  }
