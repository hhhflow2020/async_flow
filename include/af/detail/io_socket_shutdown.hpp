...
#error "io_socket_shutdown.hpp is internal to af/io_socket.hpp"
#endif

#if defined(__linux__)
template <typename TaskT>
[[nodiscard]] IoStatus io_shutdown(TaskT &task, typename TaskT::Thread thread,
                                   int fd, int how, IoOpState &state) noexcept {
  ...
  if (TaskT::Runtime::io_uring_backend_available(thread)) {
    ...
    if (TaskT::Runtime::io_submit_shutdown(thread, fd, how, &task,
                                           &state.wait)) {
      state.waiting = true;
