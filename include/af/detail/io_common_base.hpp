...
#error "io_common_base.hpp is internal to af/io_common.hpp"
#endif

namespace detail {

inline constexpr std::uint64_t io_current_offset =
...
io_timespec_from_duration(std::chrono::nanoseconds duration) noexcept {
  const auto seconds =
...
[[nodiscard]] inline bool io_apply_accepted_flags(int fd, int flags,
                                                  int &error) noexcept {
  error = 0;
#if defined(SOCK_NONBLOCK)
...
[[nodiscard]] inline bool io_fd_can_wait(int fd) noexcept {
  ...
  return S_ISSOCK(status.st_mode) || S_ISFIFO(status.st_mode) ||
