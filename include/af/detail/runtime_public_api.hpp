...
template <typename TraitsT> void AsyncRuntime<TraitsT>::shutdown() {
  AF_ASSERT(!is_runtime_thread() &&
            "shutdown must be called from a non-runtime thread");
...
void AsyncRuntime<TraitsT>::wait_for_idle() noexcept {
  for (;;) {
    const std::uint32_t count =
        unfinished_tasks_.load(std::memory_order_acquire);
    if (count == 0) {
      return;
    }
...
template <typename TraitsT>
std::uint32_t AsyncRuntime<TraitsT>::unfinished_task_count() noexcept {
...
auto AsyncRuntime<TraitsT>::make_task(CtorArgs &&...ctor_args) ->
  ...
  static_assert(std::is_base_of_v<Task, TaskT>,
                "TaskT must derive from Runtime::Task");
...
template <typename TraitsT>
bool AsyncRuntime<TraitsT>::post(Thread thread, Task *task) noexcept {
  if (task == nullptr) {
    return false;
  }
...
template <typename TraitsT>
auto AsyncRuntime<TraitsT>::current_thread() noexcept ->
    typename AsyncRuntime<TraitsT>::Thread {
  return thread_from_index(current_thread_index_);
}

template <typename TraitsT>
bool AsyncRuntime<TraitsT>::is_runtime_thread() noexcept {
...
template <typename TraitsT>
std::uint16_t AsyncRuntime<TraitsT>::current_thread_index() noexcept {
...
void AsyncRuntime<TraitsT>::parallel_shards(Thread shard_begin,
                                            ShardedOps<Op> &sharded_ops,
                                            ParallelMode mode, Task *owner,
                                            Handler &&handler) {
  parallel_shards_impl(std::bool_constant<false>{}, shard_begin, sharded_ops,
                       mode, 0, OrderedBatchOptions{}, owner,
                       std::forward<Handler>(handler));
}

template <typename TraitsT>
template <typename Op, typename Handler>
void AsyncRuntime<TraitsT>::parallel_shards(ShardedOps<Op> &sharded_ops,
                                            ParallelMode mode, Task *owner,
                                            Handler &&handler) {
  parallel_shards(thread_from_index(0), sharded_ops, mode, owner,
                  std::forward<Handler>(handler));
}

template <typename TraitsT>
template <typename Op, typename Handler>
void AsyncRuntime<TraitsT>::parallel_shards_ordered(Thread shard_begin,
                                                    ShardedOps<Op> &sharded_ops,
                                                    std::uint64_t batch_id,
                                                    Task *owner,
                                                    Handler &&handler) {
  parallel_shards_impl(std::bool_constant<true>{}, shard_begin, sharded_ops,
                       ParallelMode::AllShards, batch_id, OrderedBatchOptions{},
                       owner, std::forward<Handler>(handler));
}

template <typename TraitsT>
template <typename Op, typename Handler>
void AsyncRuntime<TraitsT>::parallel_shards_ordered(
    Thread shard_begin, ShardedOps<Op> &sharded_ops, std::uint64_t batch_id,
    OrderedBatchOptions options, Task *owner, Handler &&handler) {
  parallel_shards_impl(std::bool_constant<true>{}, shard_begin, sharded_ops,
...
void AsyncRuntime<TraitsT>::parallel_shards_ordered(ShardedOps<Op> &sharded_ops,
                                                    ...
                                                    Task *owner,
                                                    Handler &&handler) {
  parallel_shards_ordered(thread_from_index(0), sharded_ops, batch_id, options,
                          owner, std::forward<Handler>(handler));
}
...
bool AsyncRuntime<TraitsT>::start_ordered_task(Thread sequencer_thread,
                                               Batch &&batch) {
...
template <typename TraitsT>
std::uint64_t
AsyncRuntime<TraitsT>::ordered_last_applied_batch_id(Thread thread) noexcept {
  const std::uint16_t index = thread_index(thread);
