...
#error "io_file_vectored_write.hpp is internal to af/io_file.hpp"
#endif

#if !defined(_WIN32)
template <typename TaskT>
[[nodiscard]] IoStatus io_writev_at(TaskT &task, typename TaskT::Thread thread,
  ...
  if (TaskT::Runtime::io_submit_writev_at(thread, fd, iov, iov_count, offset,
                                          &task, &state.wait)) {
    state.waiting = true;
