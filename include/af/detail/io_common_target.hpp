...
io_on_target_thread(typename TaskT::Thread thread) noexcept {
  return TaskT::Runtime::is_runtime_thread() &&
         TaskT::Runtime::current_thread_index() ==
             TaskT::Runtime::thread_index(thread);
}

template <typename TaskT, typename NameFn>
[[nodiscard]] IoStatus
io_socket_name(TaskT &task, typename TaskT::Thread thread, int fd,
