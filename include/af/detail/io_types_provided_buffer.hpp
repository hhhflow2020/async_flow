...
public:
  ...
  ~IoProvidedBufferRing() { reset(); }

  [[nodiscard]] bool init(unsigned entries, int &error) noexcept {
    reset();
    error = 0;
    if (entries == 0U || (entries & (entries - 1U)) != 0U ||
        entries > max_entries()) {
      error = EINVAL;
      return false;
    }
  ...
  [[nodiscard]] bool add(const IoProvidedBuffer *buffers, unsigned count,
                         int &error) noexcept {
    error = 0;
    if (ring_ == nullptr || buffers == nullptr || count == 0U ||
        count > entries_) {
      error = EINVAL;
      return false;
    }
    ...
    auto *entries =
        reinterpret_cast<detail::IoProvidedBufferRingEntry *>(ring_);
  ...
  [[nodiscard]] void *ring() const noexcept { return ring_; }
  ...
  allocation_size(unsigned entries) noexcept {
    return sizeof(detail::IoProvidedBufferRingEntry) *
           static_cast<std::size_t>(entries);
  }

private:
  [[nodiscard]] std::uint16_t *tail() noexcept {
    return reinterpret_cast<std::uint16_t *>(static_cast<std::byte *>(ring_) +
                                             tail_offset);
  }
