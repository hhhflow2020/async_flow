...
#error "io_file_read_current.hpp is internal to af/io_file.hpp"
#endif

template <typename TaskT>
[[nodiscard]] IoStatus io_read_some(TaskT &task, typename TaskT::Thread thread,
                                    int fd, void *data, std::size_t size,
  ...
  if (!resumed_from_readiness &&
    ...
    if (TaskT::Runtime::io_submit_read_at(thread, fd, data, size,
                                          detail::io_current_offset, &task,
                                          &state.wait)) {
      state.waiting = true;
...
#if !defined(_WIN32)
template <typename TaskT>
[[nodiscard]] IoStatus io_readv_some(TaskT &task, typename TaskT::Thread thread,
  ...
  if (!resumed_from_readiness &&
      TaskT::Runtime::io_uring_backend_available(thread)) {
    state.wait = IoResult{fd, 0, 0, 0};
    if (TaskT::Runtime::io_submit_readv_at(thread, fd, iov, iov_count,
                                           detail::io_current_offset, &task,
                                           &state.wait)) {
      state.waiting = true;
      state.wait_kind = IoWaitKind::Completion;
