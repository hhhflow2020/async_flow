...
[[nodiscard]] IoStatus io_recvv_fixed_file_some(TaskT &task, typename TaskT::Thread thread,
    ...
    if (TaskT::Runtime::io_submit_recvmsg_fixed_file_iov(thread, file_index, iov, iov_count, flags,
                                                         &task, &state.wait)) {
        state.waiting = true;
...
[[nodiscard]] IoStatus io_sendv_fixed_file_some(
    TaskT &task, typename TaskT::Thread thread, int file_index, const iovec *iov, int iov_count,
    IoOpState &state,
    ...
    state.wait = IoResult{file_index, 0, 0, 0};
    if (TaskT::Runtime::io_submit_sendmsg_fixed_file_iov(thread, file_index, iov, iov_count, flags,
                                                         &task, &state.wait)) {
        state.waiting = true;
        state.wait_kind = IoWaitKind::Completion;
        return IoStatus::make_pending();
