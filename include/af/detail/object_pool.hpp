...
template <typename T, std::size_t ChunkSize = 512,
          std::size_t RemoteReleaseBatchSize = 1,
          bool CacheAllocatedSlotIndex = false,
          std::size_t LocalCacheSetSize = 8,
          std::size_t DirectReleaseSetSize = 4,
          std::size_t LocalCacheCapacity = 64>
class ObjectPool {
...
private:
    ...
    static constexpr slot_index_type null_slot_index =
        std::numeric_limits<slot_index_type>::max();
    ...
    static constexpr std::uint64_t slot_index_mask =
        (std::uint64_t{1} << slot_index_bits) - 1U;
    ...
    static constexpr std::size_t compact_storage_offset =
        align_up(slot_header_size, alignof(T));
    ...
    static constexpr std::size_t slot_size =
        align_up(slot_payload_size, slot_align);
    ...
    struct Block {
        explicit Block() {
            for (std::size_t i = 0; i < ChunkSize; ++i) {
                Slot& slot = slots[i];
                slot.owner = this;
                const auto next = i + 1U < ChunkSize
                                      ? static_cast<slot_index_type>(i + 1U)
                                      : null_slot_index;
        ...
        [[nodiscard]] std::size_t try_pop_many(Slot** out,
                                                std::size_t max_count) noexcept {
            if (max_count == 0U) {
                return 0;
            }
            AF_ASSERT(max_count <= local_cache_capacity);
            slot_index_type popped_indices[cache_allocated_slot_index
            ...
            for (;;) {
                ...
                const std::uint64_t desired =
                    pack_free_head(index, free_head_version(head) + 1U);
                if (free_head.compare_exchange_weak(
                    ...
                    if constexpr (cache_allocated_slot_index) {
                        for (std::size_t i = 0; i < count; ++i) {
                            out[i]->next_free.store(popped_indices[i],
        ...
        void push_many(Slot** pushed_slots, std::size_t count) noexcept {
            ...
            for (std::size_t i = 1; i < count; ++i) {
                pushed_slots[i - 1]->next_free.store(
                    released_slot_index(pushed_slots[i]),
                    std::memory_order_relaxed);
            }
            ...
            for (;;) {
                ...
                const std::uint64_t desired =
                    pack_free_head(first, free_head_version(head) + 1U);
                if (free_head.compare_exchange_weak(
                        head,
                        desired,
                        std::memory_order_release,
    ...
    private:
        [[nodiscard]] static constexpr std::uint64_t pack_free_head(
            slot_index_type index,
            std::uint64_t version) noexcept {
            return (version << slot_index_bits) | static_cast<std::uint64_t>(index);
        }

        [[nodiscard]] static constexpr slot_index_type free_head_index(
            std::uint64_t head) noexcept {
            return static_cast<slot_index_type>(head & slot_index_mask);
        }

        [[nodiscard]] static constexpr std::uint64_t free_head_version(
            std::uint64_t head) noexcept {
        ...
        [[nodiscard]] slot_index_type released_slot_index(const Slot* slot) const noexcept {
            if constexpr (cache_allocated_slot_index) {
                const slot_index_type index =
                    slot->next_free.load(std::memory_order_relaxed);
    ...
    struct MultiLocalCacheSet {
        static constexpr std::size_t overflow_cache_count =
            local_cache_set_size - 1U;
        ...
        [[nodiscard]] AF_DETAIL_NOINLINE LocalCache* find_release_cache_slow(
    ...
    using LocalCacheSet = std::conditional_t<local_cache_set_size == 1U,
                                             SingleLocalCacheSet,
                                             MultiLocalCacheSet>;
