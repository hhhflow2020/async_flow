...
#error "io_timeout_deadline.hpp is internal to af/io_timeout.hpp"
#endif

template <typename TaskT>
[[nodiscard]] IoStatus
arm_io_timeout(TaskT &task, typename TaskT::Thread thread, IoDeadline &deadline,
  ...
  auto consume_deadline_wait = [&]() noexcept -> IoStatus {
    ...
    return io_wait_timerfd(task, thread, deadline.timer.get(),
                           &deadline.expirations, deadline.wait);
  };
  ...
  if (TaskT::Runtime::io_backend_available(thread)) {
    const IoStatus status =
        io_wait_timeout(task, thread, deadline.delay, deadline.wait);
    if (status.pending()) {
      deadline.armed = true;
  ...
  const IoStatus status = io_wait_timerfd(task, thread, deadline.timer.get(),
                                          &deadline.expirations, deadline.wait);
  if (status.pending()) {
    deadline.armed = true;
