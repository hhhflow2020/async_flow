...
[[nodiscard]] bool
Executor<RuntimeT, TraitsT>::provided_buffer_group_registered(
...
[[nodiscard]] bool Executor<RuntimeT, TraitsT>::submit_io_uring_buffer_op(
    ...
    std::uint64_t offset, std::uint32_t op_flags, std::uint32_t complete_events,
  ...
  detail::fill_buffer_sqe(
      *sqe, detail::IoUringBufferSqe{opcode, fd, data, size, offset, op_flags},
...
[[nodiscard]] bool Executor<RuntimeT, TraitsT>::submit_io_uring_fast_sqe(
    std::uint8_t opcode, int result_fd, std::uint32_t complete_events,
    Task *task, IoResult *result, FillSqe &&fill_sqe) noexcept {
  AF_ASSERT(RuntimeT::current_thread_index_ == index_ &&
            "io_uring submit must be called from its IO thread");
  if (result != nullptr) {
    result->completion_token = nullptr;
...
template <typename RuntimeT, typename TraitsT>
[[nodiscard]] bool Executor<RuntimeT, TraitsT>::submit_io_uring_socket_impl(
    int domain, int type, int protocol, std::uint32_t flags, Task *task,
...
[[nodiscard]] bool Executor<RuntimeT, TraitsT>::submit_io_uring_fixed_file_rw(
    ...
    IoResult *result, std::uint16_t fixed_buffer_index,
...
template <typename RuntimeT, typename TraitsT>
[[nodiscard]] bool
Executor<RuntimeT, TraitsT>::validate_io_uring_generic_submit(
...
template <typename RuntimeT, typename TraitsT>
[[nodiscard]] bool
Executor<RuntimeT, TraitsT>::validate_io_uring_generic_fixed_resources(
    const IoUringGenericSubmitArgs &args,
    const IoUringGenericSubmitKind &kind) const noexcept {
  if (args.fixed_file) {
    if (kind.path_fd_op) {
...
template <typename RuntimeT, typename TraitsT>
[[nodiscard]] bool
Executor<RuntimeT, TraitsT>::attach_io_uring_generic_submit_message(
...
template <typename RuntimeT, typename TraitsT>
[[nodiscard]] bool
Executor<RuntimeT, TraitsT>::attach_io_uring_generic_submit_socket_address(
    const IoUringGenericSubmitArgs &args, const IoUringGenericSubmitKind &kind,
    IoUringOperation *operation) noexcept {
  try {
  ...
  if (kind.connect_op) {
    ...
    operation->socket_address->output_capacity =
        args.socket_address_size_out == nullptr ? 0
...
void Executor<RuntimeT, TraitsT>::fill_io_uring_generic_submit_sqe(
    io_uring_sqe &sqe, const IoUringGenericSubmitArgs &args,
...
[[nodiscard]] bool Executor<RuntimeT, TraitsT>::submit_io_uring_op(
    std::uint8_t opcode, int fd, void *data, std::size_t size,
    std::uint64_t offset, std::uint32_t op_flags, std::uint32_t complete_events,
    Task *task, IoResult *result, sockaddr *message_name,
    socklen_t message_name_len, socklen_t *message_name_len_out,
    const sockaddr *socket_address, socklen_t socket_address_size,
    sockaddr *socket_address_out, socklen_t *socket_address_size_out,
    const iovec *message_iov, std::size_t message_iov_count,
    std::uint64_t extra, std::int32_t extra_fd,
    std::uint16_t fixed_buffer_index, bool fixed_file, bool multishot,
    bool zero_copy_send, std::uint16_t provided_buffer_group,
    bool buffer_select, int direct_file_index) noexcept {
  AF_ASSERT(RuntimeT::current_thread_index_ == index_ &&
            "io_uring submit must be called from its IO thread");
  if (result != nullptr) {
    result->completion_token = nullptr;
