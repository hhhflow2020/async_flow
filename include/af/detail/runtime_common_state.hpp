...
template <typename T> struct alignas(hardware_cache_line_size) CacheLineAtomic {
  ...
  void notify_one() noexcept { value.notify_one(); }
...
template <typename RuntimeT> struct RuntimeParallelGroup {
  ...
  void complete(bool ok, bool resume_owner = true) noexcept {
    ...
    if (pending.fetch_sub(1, std::memory_order_acq_rel) == 1U) {
      if (resume_owner && owner != nullptr &&
          resume_thread < RuntimeT::thread_count) {
        RuntimeT::set_task_parallel_failures(
            owner, failed.load(std::memory_order_acquire));
        RuntimeT::post_blocking(RuntimeT::thread_from_index(resume_thread),
                                owner);
      }
