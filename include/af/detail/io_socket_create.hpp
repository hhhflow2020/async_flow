...
#error "io_socket_create.hpp is internal to af/io_socket.hpp"
#endif

template <typename TaskT>
[[nodiscard]] IoStatus io_socket(TaskT &task, typename TaskT::Thread thread,
                                 int domain, int type, int protocol,
                                 int *opened_fd, IoOpState &state,
  ...
  *opened_fd = -1;
  ...
  if (TaskT::Runtime::io_uring_backend_available(thread)) {
    ...
    if (TaskT::Runtime::io_submit_socket(thread, domain, type, protocol, flags,
                                         &task, &state.wait)) {
      state.waiting = true;
