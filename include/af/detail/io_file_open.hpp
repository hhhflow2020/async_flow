...
#error "io_file_open.hpp is internal to af/io_file.hpp"
#endif

template <typename TaskT>
[[nodiscard]] IoStatus io_openat(TaskT &task, typename TaskT::Thread thread,
                                 int dir_fd, const char *path, int flags,
                                 std::uint32_t mode, int *opened_fd,
  ...
  *opened_fd = -1;
  ...
  if (TaskT::Runtime::io_submit_openat(thread, dir_fd, path, flags, mode, &task,
                                       &state.wait)) {
    state.waiting = true;
...
template <typename TaskT>
[[nodiscard]] IoStatus
io_openat_direct(TaskT &task, typename TaskT::Thread thread, int dir_fd,
                 const char *path, int flags, std::uint32_t mode,
                 int file_index,
  ...
  if (TaskT::Runtime::io_submit_openat_direct(thread, dir_fd, path, flags, mode,
                                              file_index, &task, &state.wait)) {
    state.waiting = true;
    state.wait_kind = IoWaitKind::Completion;
    return IoStatus::make_pending();
  }
