...
#error "io_datagram_zero_copy.hpp is internal to af/io_datagram.hpp"
#endif

template <typename TaskT>
[[nodiscard]] IoStatus
io_send_zc_to_some(TaskT &task, typename TaskT::Thread thread, int fd,
  ...
  if (!skip_uring && !resumed_from_readiness &&
    ...
    if (TaskT::Runtime::io_submit_sendmsg_zc(
            thread, fd, data, size, address, address_size,
  ...
  for (;;) {
    const ssize_t n = ::sendto(fd, data, size, detail::io_no_signal_flag(),
...
template <typename TaskT>
[[nodiscard]] IoStatus
io_sendv_zc_to_some(TaskT &task, typename TaskT::Thread thread, int fd,
  ...
  if (!skip_uring && !resumed_from_readiness &&
    ...
    if (TaskT::Runtime::io_submit_sendmsg_zc_iov(
            thread, fd, iov, iov_count, address, address_size,
            static_cast<std::uint32_t>(detail::io_no_signal_flag()), &task,
            &state.wait)) {
      state.waiting = true;
      state.wait_kind = IoWaitKind::Completion;
