...
[[nodiscard]] IoStatus io_send_fixed_file_some(
    TaskT &task, typename TaskT::Thread thread, int file_index, const void *data, std::size_t size,
    IoOpState &state,
    ...
    if (TaskT::Runtime::io_submit_send_fixed_file(thread, file_index, data, size, flags, &task,
                                                  &state.wait)) {
        state.waiting = true;
