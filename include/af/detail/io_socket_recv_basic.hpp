...
#error "io_socket_recv_basic.hpp is internal to af/io_socket.hpp"
#endif

template <typename TaskT>
[[nodiscard]] IoStatus io_recv_some(TaskT &task, typename TaskT::Thread thread,
  ...
  if (!resumed_from_readiness &&
    ...
    if (TaskT::Runtime::io_submit_recv(thread, fd, data, size, 0, &task,
                                       &state.wait)) {
      state.waiting = true;
...
template <typename TaskT>
[[nodiscard]] IoStatus
io_recv_fixed_file_some(TaskT &task, typename TaskT::Thread thread,
  ...
  if (TaskT::Runtime::io_submit_recv_fixed_file(thread, file_index, data, size,
