...
[[nodiscard]] inline bool io_parse_recvmsg_multishot_buffer(
    const void *buffer, std::size_t buffer_size, std::size_t received_size, socklen_t name_capacity,
    std::size_t control_capacity, IoRecvmsgMultishotView &view, int &error) noexcept {
    view = IoRecvmsgMultishotView{};
    error = 0;
    if (buffer == nullptr || received_size > buffer_size ||
    ...
    view.name_size =
        static_cast<std::uint32_t>(std::min<std::size_t>(out->namelen, name_capacity_size));
    view.control_offset = static_cast<std::uint32_t>(control_offset);
    view.control_size =
        static_cast<std::uint32_t>(std::min<std::size_t>(out->controllen, control_capacity));
    view.payload_offset = static_cast<std::uint32_t>(payload_offset);
    view.payload_size =
        static_cast<std::uint32_t>(std::min<std::size_t>(out->payloadlen, payload_available));
...
[[nodiscard]] IoStatus io_recvmsg_multishot(TaskT &task, typename TaskT::Thread thread, int fd,
                                            std::uint16_t buffer_group, socklen_t name_capacity,
                                            std::size_t control_capacity, std::uint16_t *buffer_id,
    ...
    *buffer_id = 0;
    ...
    if (TaskT::Runtime::io_submit_recvmsg_multishot(thread, fd, buffer_group, name_capacity,
                                                    control_capacity, flags, &task, &state.wait)) {
        state.waiting = true;
