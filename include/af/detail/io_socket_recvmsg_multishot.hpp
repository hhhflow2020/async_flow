...
#error "io_socket_recvmsg_multishot.hpp is internal to af/io_socket.hpp"
#endif

#if defined(__linux__)
[[nodiscard]] inline bool io_parse_recvmsg_multishot_buffer(
    const void *buffer, std::size_t buffer_size, std::size_t received_size,
    socklen_t name_capacity, std::size_t control_capacity,
    IoRecvmsgMultishotView &view, int &error) noexcept {
  ...
  const std::size_t name_capacity_size =
      static_cast<std::size_t>(name_capacity);
...
[[nodiscard]] IoStatus
io_recvmsg_multishot(TaskT &task, typename TaskT::Thread thread, int fd,
  ...
  *buffer_id = 0;
  ...
  if (TaskT::Runtime::io_submit_recvmsg_multishot(
          thread, fd, buffer_group, name_capacity, control_capacity, flags,
          &task, &state.wait)) {
    state.waiting = true;
