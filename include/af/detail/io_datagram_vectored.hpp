...
#error "io_datagram_vectored.hpp is internal to af/io_datagram.hpp"
#endif

#if !defined(_WIN32)
template <typename TaskT>
[[nodiscard]] IoStatus
io_recvv_from_some(TaskT &task, typename TaskT::Thread thread, int fd,
...
template <typename TaskT>
[[nodiscard]] IoStatus
io_sendv_to_some(TaskT &task, typename TaskT::Thread thread, int fd,
  ...
  if (!resumed_from_readiness &&
    ...
    if (TaskT::Runtime::io_submit_sendmsg_iov(
            ...
            detail::io_no_signal_flag(), &task, &state.wait)) {
      state.waiting = true;
      state.wait_kind = IoWaitKind::Completion;
      return IoStatus::make_pending();
    }
