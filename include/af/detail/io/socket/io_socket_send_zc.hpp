...
[[nodiscard]] IoStatus io_send_zc_some(TaskT &task, typename TaskT::Thread thread, int fd,
    ...
    if (!skip_uring && !resumed_from_readiness &&
        ...
        if (TaskT::Runtime::io_submit_send_zc(
                thread, fd, data, size, static_cast<std::uint32_t>(detail::io_no_signal_flag()),
                &task, &state.wait)) {
            state.waiting = true;
