...
[[nodiscard]] IoStatus io_accept_multishot(TaskT &task, typename TaskT::Thread thread, int fd,
    ...
    *accepted_fd = -1;
    ...
    if (TaskT::Runtime::io_submit_accept_multishot(thread, fd, address, address_size, flags, &task,
                                                   &state.wait)) {
        state.waiting = true;
