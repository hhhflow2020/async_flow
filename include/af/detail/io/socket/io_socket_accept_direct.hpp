...
[[nodiscard]] IoStatus
io_accept_direct(TaskT &task, typename TaskT::Thread thread, int fd, sockaddr *address,
                 socklen_t *address_size, int flags, int file_index,
                 IoFixedFile<typename TaskT::Thread> *accepted_file, IoOpState &state) noexcept {
    ...
    if (TaskT::Runtime::io_submit_accept_direct(thread, fd, address, address_size, flags,
