...
[[nodiscard]] IoStatus io_fsync_fixed_file(TaskT &task, typename TaskT::Thread thread,
                                           int file_index, IoOpState &state,
    ...
    if (TaskT::Runtime::io_submit_fsync_fixed_file(thread, file_index, flags, &task, &state.wait)) {
        state.waiting = true;
...
[[nodiscard]] IoStatus io_read_fixed_file_at(TaskT &task, typename TaskT::Thread thread,
    ...
    state.wait = IoResult{file_index, 0, 0, 0};
    if (TaskT::Runtime::io_submit_read_fixed_file_at(thread, file_index, data, size, offset,
                                                     buffer_index, &task, &state.wait)) {
        state.waiting = true;
        state.wait_kind = IoWaitKind::Completion;
        return IoStatus::make_pending();
    }
...
[[nodiscard]] IoStatus io_read_fixed_file_at(TaskT &task, typename TaskT::Thread thread,
    ...
    return io_read_fixed_file_at(task, thread, file_index, buffer.data, buffer.size, offset,
                                 buffer.index, state);
}

template <typename TaskT>
[[nodiscard]] IoStatus io_write_fixed_file_at(TaskT &task, typename TaskT::Thread thread,
    ...
    state.wait = IoResult{file_index, 0, 0, 0};
    if (TaskT::Runtime::io_submit_write_fixed_file_at(thread, file_index, data, size, offset,
                                                      buffer_index, &task, &state.wait)) {
        state.waiting = true;
        state.wait_kind = IoWaitKind::Completion;
        return IoStatus::make_pending();
