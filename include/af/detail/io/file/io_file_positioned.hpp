...
[[nodiscard]] IoStatus io_read_fixed_file_at(TaskT &task, typename TaskT::Thread thread,
    ...
    if (TaskT::Runtime::io_submit_read_fixed_file_at(thread, file_index, data, size, offset, &task,
                                                     &state.wait)) {
        state.waiting = true;
        state.wait_kind = IoWaitKind::Completion;
        return IoStatus::make_pending();
...
[[nodiscard]] IoStatus io_write_fixed_file_at(TaskT &task, typename TaskT::Thread thread,
    ...
    state.wait = IoResult{file_index, 0, 0, 0};
    if (TaskT::Runtime::io_submit_write_fixed_file_at(thread, file_index, data, size, offset, &task,
                                                      &state.wait)) {
        state.waiting = true;
        state.wait_kind = IoWaitKind::Completion;
        return IoStatus::make_pending();
...
[[nodiscard]] IoStatus io_readv_fixed_file_at(TaskT &task, typename TaskT::Thread thread,
    ...
    state.wait = IoResult{file_index, 0, 0, 0};
    if (TaskT::Runtime::io_submit_readv_fixed_file_at(thread, file_index, iov, iov_count, offset,
                                                      &task, &state.wait)) {
        state.waiting = true;
        state.wait_kind = IoWaitKind::Completion;
        return IoStatus::make_pending();
...
template <typename TaskT>
[[nodiscard]] IoStatus io_writev_fixed_file_at(TaskT &task, typename TaskT::Thread thread,
                                               int file_index, const iovec *iov, int iov_count,
                                               std::uint64_t offset, IoOpState &state) noexcept {
    if (detail::waiting_for_completion(state)) {
        return detail::completed_uring_status(state);
    ...
    state.wait = IoResult{file_index, 0, 0, 0};
    if (TaskT::Runtime::io_submit_writev_fixed_file_at(thread, file_index, iov, iov_count, offset,
                                                       &task, &state.wait)) {
        state.waiting = true;
        state.wait_kind = IoWaitKind::Completion;
        return IoStatus::make_pending();
