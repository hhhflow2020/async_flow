...
public:
    ...
    [[nodiscard]] bool init(unsigned entries, int &error) noexcept {
        reset();
        error = 0;
        if (entries == 0U || (entries & (entries - 1U)) != 0U || entries > max_entries()) {
            error = EINVAL;
            return false;
        }
...
private:
    [[nodiscard]] std::uint16_t *tail() noexcept {
        return reinterpret_cast<std::uint16_t *>(static_cast<std::byte *>(ring_) + tail_offset);
    }

    [[nodiscard]] const std::uint16_t *tail() const noexcept {
        return reinterpret_cast<const std::uint16_t *>(static_cast<const std::byte *>(ring_) +
                                                       tail_offset);
    }
