...
#error "io_datagram_recv.hpp is internal to af/io_datagram.hpp"
#endif

template <typename TaskT>
[[nodiscard]] IoStatus
io_recv_from_some(TaskT &task, typename TaskT::Thread thread, int fd,
  ...
  if (!resumed_from_readiness &&
    ...
    if (TaskT::Runtime::io_submit_recvmsg(thread, fd, data, size, address,
                                          address_size, 0, &task,
                                          &state.wait)) {
      state.waiting = true;
