...
template <typename TraitsT> void AsyncRuntime<TraitsT>::init_queues() {
  ...
  for (std::uint16_t target = 0; target < thread_count; ++target) {
    static_cast<void>(target);
    external_queues_.push_back(
        std::make_unique<ExternalQueue>(external_queue_capacity));
  }
}

template <typename TraitsT>
auto AsyncRuntime<TraitsT>::spsc_queue(std::uint16_t source,
                                       std::uint16_t target) noexcept
    -> SpscQueue & {
  return *spsc_queues_[static_cast<std::size_t>(source) * thread_count +
                       target];
}

template <typename TraitsT>
constexpr auto AsyncRuntime<TraitsT>::ready_route_from_runtime_thread(
    std::uint16_t source, std::uint16_t target) noexcept -> ReadyQueueRoute {
  return source == target ? ReadyQueueRoute::Local : ReadyQueueRoute::Spsc;
}

template <typename TraitsT>
bool AsyncRuntime<TraitsT>::try_enqueue_ready(std::uint16_t index,
                                              Task *task) noexcept {
  if (current_thread_index_ < thread_count) {
    return try_enqueue_ready_from_runtime_thread(current_thread_index_, index,
                                                 task);
  }
...
template <typename TraitsT>
bool AsyncRuntime<TraitsT>::try_enqueue_ready_from_runtime_thread(
    std::uint16_t source, std::uint16_t target, Task *task) noexcept {
...
template <typename TraitsT>
bool AsyncRuntime<TraitsT>::try_enqueue_local_from_runtime_thread(
    std::uint16_t target, Task *task) noexcept {
...
template <typename TraitsT>
bool AsyncRuntime<TraitsT>::try_enqueue_external_mpsc(std::uint16_t target,
                                                      Task *task) noexcept {
...
template <typename TraitsT>
void AsyncRuntime<TraitsT>::mark_source_ready(std::uint16_t source,
                                              std::uint16_t target) noexcept {
  executors_[target]->mark_ready(source);
}

template <typename TraitsT>
void AsyncRuntime<TraitsT>::enqueue_ready_blocking(std::uint16_t index,
                                                   Task *task) noexcept {
  if (current_thread_index_ < thread_count) {
    enqueue_ready_blocking_from_runtime_thread(current_thread_index_, index,
                                               task);
    return;
  }
...
template <typename TraitsT>
void AsyncRuntime<TraitsT>::enqueue_ready_blocking_from_runtime_thread(
    std::uint16_t source, std::uint16_t target, Task *task) noexcept {
  const ReadyQueueRoute route = ready_route_from_runtime_thread(source, target);
  if (route == ReadyQueueRoute::Local) {
...
template <typename TraitsT>
void AsyncRuntime<TraitsT>::enqueue_local_from_runtime_thread_blocking(
    std::uint16_t target, Task *task) noexcept {
...
template <typename TraitsT>
void AsyncRuntime<TraitsT>::enqueue_cross_thread_spsc_blocking(
    std::uint16_t source, std::uint16_t target, Task *task) noexcept {
...
template <typename TraitsT>
void AsyncRuntime<TraitsT>::enqueue_external_mpsc_blocking(
    std::uint16_t target, Task *task) noexcept {
...
template <typename TraitsT>
bool AsyncRuntime<TraitsT>::enqueue_ready_by_policy(std::uint16_t index,
                                                    Task *task) noexcept {
...
template <typename TraitsT>
void AsyncRuntime<TraitsT>::enqueue_pending_blocking(std::uint16_t index,
                                                     Task *task) noexcept {
  TaskState expected = TaskState::Pending;
  if (task->state_.compare_exchange_strong(expected, TaskState::Queued,
...
bool AsyncRuntime<TraitsT>::try_enter_post(std::uint16_t target) noexcept {
  ...
  if (status == RuntimeStatus::Running) {
    active_external_posts_[target].value.fetch_add(1,
                                                   std::memory_order_acq_rel);
...
template <typename TraitsT>
void AsyncRuntime<TraitsT>::leave_post(std::uint16_t target) noexcept {
  if (current_thread_index_ < thread_count) {
    return;
  }

  AF_ASSERT(target < thread_count);
  if (active_external_posts_[target].value.fetch_sub(
...
template <typename TraitsT>
void AsyncRuntime<TraitsT>::wait_for_external_posts() noexcept {
