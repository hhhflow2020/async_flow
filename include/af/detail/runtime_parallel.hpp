...
void AsyncRuntime<TraitsT>::parallel_shards_impl(
    std::bool_constant<Ordered>, Thread shard_begin,
    ShardedOps<Op> &sharded_ops, ParallelMode mode, std::uint64_t batch_id,
    OrderedBatchOptions ordered_options, Task *owner, Handler &&handler) {
  AF_ASSERT(owner != nullptr);
  AF_ASSERT(is_runtime_thread() &&
            "parallel_shards must be called from a runtime thread");
  ...
  auto *group =
      create_parallel_group(target_count, owner, current_thread_index());
  ...
  for (std::uint16_t i = 0; i < shard_count; ++i) {
    ...
    const Thread thread =
        thread_from_index(static_cast<std::uint16_t>(begin + i));
    if (thread_index(thread) == current_thread_index()) {
      ...
      const bool ok = run_parallel_shard<Op, HandlerT, Ordered>(
          i, batch_id, ordered_options, ops, local_handler);
      group->complete(ok);
      continue;
    }
...
template <typename TraitsT>
auto AsyncRuntime<TraitsT>::check_order_guard(
    std::uint64_t batch_id, OrderedBatchOptions options) noexcept
...
template <typename TraitsT>
void AsyncRuntime<TraitsT>::commit_order_guard(
...
struct AsyncRuntime<TraitsT>::OrderedStartState {
  ...
  [[nodiscard]] bool start_ready(BatchT batch) {
    const bool ok =
        AsyncRuntime::template start_task<ApplyTaskT>(std::move(batch));
    if (!ok) {
      return false;
    }
    ++next_batch_id;
...
template <typename TraitsT>
template <typename StreamTag, typename ApplyTaskT, typename BatchT>
auto AsyncRuntime<TraitsT>::ordered_start_state() -> typename AsyncRuntime<
    TraitsT>::template OrderedStartState<StreamTag, ApplyTaskT, BatchT> & {
  static std::vector<OrderedStartState<StreamTag, ApplyTaskT, BatchT>> states(
      thread_count);
...
template <typename TraitsT>
template <typename StreamTag, typename ApplyTaskT, typename BatchT>
class AsyncRuntime<TraitsT>::OrderedStartTask final
    : public AsyncRuntime<TraitsT>::Task {
public:
  using RuntimeTask = typename AsyncRuntime<TraitsT>::Task;

  explicit OrderedStartTask(typename RuntimeTask::FactoryToken token)
      : RuntimeTask(token) {}
...
template <typename TraitsT>
template <typename Op, typename Handler, bool Ordered>
bool AsyncRuntime<TraitsT>::run_parallel_shard(std::uint16_t shard_index,
...
template <typename TraitsT>
template <typename Op, typename Handler, bool Ordered>
class AsyncRuntime<TraitsT>::ShardTask final
    : public AsyncRuntime<TraitsT>::Task {
public:
  using RuntimeTask = typename AsyncRuntime<TraitsT>::Task;

  ShardTask(typename RuntimeTask::FactoryToken token, ParallelGroup *group,
            std::uint16_t shard_index, std::uint64_t batch_id,
            OrderedBatchOptions options, std::vector<Op> &&ops, Handler handler)
      : RuntimeTask(token), group_(group), shard_index_(shard_index),
        batch_id_(batch_id), options_(options), ops_(std::move(ops)),
...
private:
  ...
  void on_runtime_cancel() noexcept override { group_->complete(false, false); }
