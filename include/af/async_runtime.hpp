...
class AsyncRuntime
    : public detail::RuntimePublicIo<AsyncRuntime<TraitsT>, TraitsT> {
public:
  ...
  using TaskHandle = detail::RuntimeTaskHandle<AsyncRuntime, TaskT>;

  template <typename RuntimeT, typename TaskT>
  friend class detail::RuntimeTaskHandle;

  template <typename RuntimeT> friend struct detail::RuntimeParallelGroup;

  template <typename RuntimeT, typename TraitsForRuntime>
  friend struct detail::RuntimePublicIo;
  ...
  thread_index(Thread thread) noexcept {
  ...
  static void parallel_shards(Thread shard_begin, ShardedOps<Op> &sharded_ops,
                              ParallelMode mode, Task *owner,
                              Handler &&handler);
  ...
  template <typename Op, typename Handler>
  static void parallel_shards_ordered(Thread shard_begin,
  ...
  template <typename Op, typename Handler>
  static void
  parallel_shards_ordered(Thread shard_begin, ShardedOps<Op> &sharded_ops,
                          std::uint64_t batch_id, OrderedBatchOptions options,
                          Task *owner, Handler &&handler);

  template <typename Op, typename Handler>
  static void parallel_shards_ordered(ShardedOps<Op> &sharded_ops,
                                      std::uint64_t batch_id, Task *owner,
                                      Handler &&handler);
  ...
  [[nodiscard]] static bool start_ordered_task(Thread sequencer_thread,
                                               Batch &&batch);

  [[nodiscard]] static std::uint64_t
  ordered_last_applied_batch_id(Thread thread) noexcept;
...
private:
  ...
  static void set_task_parallel_failures(Task *task,
  ...
  static void
  parallel_shards_impl(std::bool_constant<Ordered> ordered, Thread shard_begin,
  ...
  check_order_guard(std::uint64_t batch_id,
  ...
  template <typename StreamTag, typename ApplyTaskT, typename BatchT>
  [[nodiscard]] static OrderedStartState<StreamTag, ApplyTaskT, BatchT> &
  ordered_start_state();

  template <typename StreamTag, typename ApplyTaskT, typename Batch>
  class OrderedStartTask;

  template <typename Op, typename Handler, bool Ordered>
  [[nodiscard]] static bool
  run_parallel_shard(std::uint16_t shard_index, std::uint64_t batch_id,
  ...
  create_parallel_group(std::uint32_t target_count, Task *owner,
  ...
  [[nodiscard]] static SpscQueue &spsc_queue(std::uint16_t source,
                                             std::uint16_t target) noexcept;

  [[nodiscard]] static constexpr ReadyQueueRoute
  ready_route_from_runtime_thread(std::uint16_t source,
                                  std::uint16_t target) noexcept;

  static bool try_enqueue_ready(std::uint16_t index, Task *task) noexcept;
  static bool try_enqueue_ready_from_runtime_thread(std::uint16_t source,
                                                    std::uint16_t target,
                                                    Task *task) noexcept;
  static bool try_enqueue_local_from_runtime_thread(std::uint16_t target,
                                                    Task *task) noexcept;
  static bool try_enqueue_cross_thread_spsc(std::uint16_t source,
                                            std::uint16_t target,
                                            Task *task) noexcept;
  static bool try_enqueue_external_mpsc(std::uint16_t target,
                                        Task *task) noexcept;
  static void mark_source_ready(std::uint16_t source,
                                std::uint16_t target) noexcept;
  ...
  static void enqueue_local_from_runtime_thread_blocking(std::uint16_t target,
                                                         Task *task) noexcept;
  static void enqueue_cross_thread_spsc_blocking(std::uint16_t source,
                                                 std::uint16_t target,
                                                 Task *task) noexcept;
  static void enqueue_external_mpsc_blocking(std::uint16_t target,
                                             Task *task) noexcept;
  ...
  static inline std::array<ExternalPostCounter, thread_count>
      active_external_posts_{};
  ...
  static inline thread_local std::uint16_t current_thread_index_ =
