...
public:
    ...
    TcpConnection(Shard *shard, int fd, std::uint32_t slot, std::uint32_t generation) noexcept
...
private:
        ...
        while (alive() && consumed < shard_options().read_budget_bytes) {
            ...
            if (n > 0) {
                ...
                if constexpr (requires(Handler h, TcpConnectionRef<Runtime, Handler> c,
                    ...
                    handler().on_read(ref,
                                      af::BufferView(buffer.data(), static_cast<std::size_t>(n)));
                }
                continue;
            }
            if (n == 0) {
    ...
    void update_interest() noexcept {
        ...
        std::uint32_t events = af::detail::net_io_readable;
        if (!output_.empty()) {
...
private:
    ...
    [[nodiscard]] bool open_listener(const State &state) noexcept {
        ...
        address.sin_port = htons(state.endpoint.port);
        if (::inet_pton(AF_INET, state.endpoint.address.c_str(), &address.sin_addr) != 1) {
            close_listener();
            return false;
        }
        if (::bind(listener_fd_, reinterpret_cast<const sockaddr *>(&address), sizeof(address)) !=
            0) {
            close_listener();
            return false;
        }
    ...
    void create_connection(int fd) noexcept {
        const std::uint32_t slot = static_cast<std::uint32_t>(connections_.size());
        generations_.push_back(1U);
        auto connection = std::make_unique<Connection>(this, fd, slot, generations_.back());
...
public:
    ...
    [[nodiscard]] bool close() const {
        ...
        auto command = std::make_unique<Command>();
        command->kind = Command::Kind::Close;
        command->slot = slot_;
        command->generation = generation_;
        if (!state->shards[shard_index_]->enqueue(command.get())) {
            return false;
