...
TEST_F(ParallelRuntimeFixture, OrderedBatchConvenienceOverloadRunsAllShards) {
    ...
    for (std::uint16_t i = 0; i < 4; ++i) {
        ...
        EXPECT_EQ(Runtime::ordered_last_applied_batch_id(Runtime::thread_from_index(i)), 1U);
    }
}
...
TEST_F(ParallelRuntimeFixture, RetryableOrderedBatchSkipsAlreadyAppliedShards) {
    ...
    ASSERT_TRUE(
        Runtime::start_task<OrderedFailureTask>(1U, 1U, &failed_completed, &failed_failures));
    ...
    ASSERT_TRUE(Runtime::start_task<OrderedRetryableTask>(1U, &retry_completed, &shard_hits,
                                                          &retry_failures));
