...
TEST_F(UringIoRuntimeSocketMultishotFixture, IoUringUdpRecvmsgMultishotReportsPeerAddress) {
    ...
    ASSERT_EQ(::getsockname(sender.get(), reinterpret_cast<sockaddr *>(&sender_address),
                            &sender_address_size),
              0);
    ...
    ASSERT_TRUE(UringIoRuntime::start_task<UringUdpRecvmsgMultishotTask>(
        receiver.get(), sender_address.sin_port, &armed, &completed, &read_count, &packed_read,
        &peer_count, &error));
    ...
    ASSERT_EQ(::sendto(sender.get(), payload, 1, 0, reinterpret_cast<sockaddr *>(&receiver_address),
                       receiver_address_size),
              1);
    ASSERT_EQ(::sendto(sender.get(), payload + 1, 1, 0,
                       reinterpret_cast<sockaddr *>(&receiver_address), receiver_address_size),
              1);
