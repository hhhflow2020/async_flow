...
TEST_F(ParallelRuntimeFixture, OrderedStartTaskBuffersOutOfOrderBatches) {
    ...
    ASSERT_TRUE((Runtime::start_ordered_task<OrderedStartStream, OrderedStartApplyTask>(
        TestThreads::DB_0, OrderedStartBatch{2, 20, &applied, &completed})));
    ASSERT_TRUE((Runtime::start_ordered_task<OrderedStartStream, OrderedStartApplyTask>(
        TestThreads::DB_0, OrderedStartBatch{1, 10, &applied, &completed})));
    ASSERT_TRUE((Runtime::start_ordered_task<OrderedStartStream, OrderedStartApplyTask>(
        TestThreads::DB_0, OrderedStartBatch{3, 30, &applied, &completed})));
...
TEST_F(ParallelRuntimeFixture, OrderedStartTaskIgnoresDuplicateAndOldBatches) {
    ...
    ASSERT_TRUE((Runtime::start_ordered_task<OrderedStartStream, OrderedStartApplyTask>(
        TestThreads::DB_0, OrderedStartBatch{1, 100, &applied, &completed})));
    ASSERT_TRUE((Runtime::start_ordered_task<OrderedStartStream, OrderedStartApplyTask>(
        TestThreads::DB_0, OrderedStartBatch{2, 20, &applied, &completed})));
    ASSERT_TRUE(wait_until_at_least(completed, 2));
...
TEST(RuntimeOrderedStartTests, OrderedStartStateResetsAfterRuntimeRestart) {
    ...
    ASSERT_TRUE((Runtime::start_ordered_task<OrderedStartStream, OrderedStartApplyTask>(
        TestThreads::DB_0, OrderedStartBatch{1, 10, &first_applied, &first_completed})));
    ...
    ASSERT_TRUE((Runtime::start_ordered_task<OrderedStartStream, OrderedStartApplyTask>(
        TestThreads::DB_0, OrderedStartBatch{1, 20, &second_applied, &second_completed})));
