...
private:
    af::TaskResult run() override {
        ...
        const af::IoStatus invalid_recvmsg_datagram_status = invalid_datagram.recv_from_multishot(
            *this, 0, sizeof(sockaddr_storage), 0, &buffer_id, invalid_recvmsg_datagram_state);
        const af::IoStatus null_status = stream_.recv_multishot(*this, 0, nullptr, null_state);
        const af::IoStatus null_datagram_status =
            datagram_.recv_multishot(*this, 0, nullptr, null_datagram_state);
        const af::IoStatus null_recvmsg_datagram_status = datagram_.recv_from_multishot(
            *this, 0, sizeof(sockaddr_storage), 0, nullptr, null_recvmsg_datagram_state);
        ...
        const af::IoStatus unavailable_recvmsg_datagram_status = datagram_.recv_from_multishot(
            *this, 0, sizeof(sockaddr_storage), 0, &buffer_id, unavailable_recvmsg_datagram_state);
        if (!invalid_status.failed() || invalid_status.error != EBADF ||
            ...
            invalid_recvmsg_datagram_status.error != EBADF || !null_status.failed() ||
            null_status.error != EINVAL || !null_datagram_status.failed() ||
            null_datagram_status.error != EINVAL || !null_recvmsg_datagram_status.failed() ||
            null_recvmsg_datagram_status.error != EINVAL || !unavailable_status.failed() ||
            unavailable_status.error != ENOSYS) {
            return failed();
        }
        if (!unavailable_datagram_status.failed() || unavailable_datagram_status.error != ENOSYS ||
        ...
        if (!af::io_parse_recvmsg_multishot_buffer(raw_buffer, sizeof(raw_buffer), received_size,
                                                   name_capacity, 0, view, parse_error) ||
            view.name_offset != sizeof(af::detail::IoUringRecvmsgOut) ||
            view.name_size != sizeof(sockaddr_in) || view.payload_offset != received_size - 1U ||
            view.payload_size != 1U) {
            return failed();
        }
        if (af::io_parse_recvmsg_multishot_buffer(nullptr, sizeof(raw_buffer), received_size,
                                                  name_capacity, 0, view, parse_error) ||
            parse_error != EINVAL) {
            return failed();
        }
