...
template <typename TaskBaseT> class BasicTimerFdTask final : public TaskBaseT {
public:
...
template <typename TaskBaseT> class BasicUringTimeoutTask final : public TaskBaseT {
public:
    explicit BasicUringTimeoutTask(typename TaskBaseT::FactoryToken token) : TaskBaseT(token) {}

    bool do_it(std::atomic<int> *armed, std::atomic<int> *completed, std::atomic<int> *error) {
        armed_ = armed;
        completed_ = completed;
        error_ = error;
        return this->schedule(IoTestThreads::IO_0);
