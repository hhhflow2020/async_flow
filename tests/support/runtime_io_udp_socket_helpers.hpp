...
bool create_udp_loopback_sockets(UdpLoopbackSockets &sockets) {
    ...
    if (::bind(sockets.receiver.get(), reinterpret_cast<sockaddr *>(&sockets.address),
               sizeof(sockets.address)) != 0) {
        return false;
    }

    sockets.address_size = sizeof(sockets.address);
    return ::getsockname(sockets.receiver.get(), reinterpret_cast<sockaddr *>(&sockets.address),
...
bool connect_udp_loopback_sockets(UdpLoopbackSockets &sockets) {
    ...
    if (::bind(sockets.sender.get(), reinterpret_cast<sockaddr *>(&sender_address),
               sizeof(sender_address)) != 0) {
        return false;
    }

    socklen_t sender_address_size = sizeof(sender_address);
    if (::getsockname(sockets.sender.get(), reinterpret_cast<sockaddr *>(&sender_address),
                      &sender_address_size) != 0) {
        return false;
    }

    if (::connect(sockets.receiver.get(), reinterpret_cast<sockaddr *>(&sender_address),
                  sender_address_size) != 0) {
        return false;
    }
    return ::connect(sockets.sender.get(), reinterpret_cast<sockaddr *>(&sockets.address),
                     sockets.address_size) == 0;
}

ssize_t send_udp_payload(const UdpLoopbackSockets &sockets, const void *data, std::size_t size) {
    return ::sendto(sockets.sender.get(), data, size, 0,
                    reinterpret_cast<const sockaddr *>(&sockets.address), sockets.address_size);
}

ssize_t recv_udp_payload(const UdpLoopbackSockets &sockets, void *data, std::size_t size) {
    sockaddr_storage peer{};
    socklen_t peer_size = sizeof(peer);
    return ::recvfrom(sockets.receiver.get(), data, size, 0, reinterpret_cast<sockaddr *>(&peer),
                      &peer_size);
}
