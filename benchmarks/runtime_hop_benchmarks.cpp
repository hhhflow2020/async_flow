...
template <std::size_t TaskPoolRemoteReleaseBatchSize,
          bool TaskPoolCacheSlotIndex,
          std::size_t TaskPoolChunkSize = 256,
          std::size_t TaskPoolLocalCacheSetSize = 1,
          std::size_t TaskPoolDirectReleaseSetSize = 4,
          std::size_t TaskPoolLocalCacheCapacity = 64>
...
private:
    af::TaskResult run() override {
        if (hops_-- > 0) {
            const auto next = RuntimeT::current_thread() == Thread::Logic_0
                                  ? Thread::Logic_1
                                  : Thread::Logic_0;
...
void BM_RuntimeCrossThreadHopTaskPoolBatch(benchmark::State& state) {
    using Runtime = af::AsyncRuntime<
        BatchRuntimeTraits<TaskPoolRemoteReleaseBatchSize,
                           TaskPoolCacheSlotIndex,
                           TaskPoolChunkSize,
                           TaskPoolLocalCacheSetSize,
                           TaskPoolDirectReleaseSetSize,
                           TaskPoolLocalCacheCapacity>>;
    ...
    for (auto _ : state) {
        ...
        for (int i = 0; i < task_count; ++i) {
            remaining.fetch_add(1, std::memory_order_relaxed);
            const bool ok =
                Runtime::template start_task<BatchHopTask<Runtime>>(8, &remaining);
            if (!ok) {
                af_bench::runtime::undo_remaining(remaining);
