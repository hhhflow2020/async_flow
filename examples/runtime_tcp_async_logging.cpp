...
class runtime_tcp_log_task final : public af::runtime_task {
public:
    runtime_tcp_log_task(af::runtime_task::factory_token token, af::runtime &owner) noexcept
...
int main() {
    ...
    for (std::uint32_t i = 0; i < static_cast<std::uint32_t>(task_count); ++i) {
        auto task = af::make_task<runtime_tcp_log_task>(runtime);
