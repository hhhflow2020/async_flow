...
- `examples/io_uring_fixed_file.cpp`：使用 `async::io_register_files()` + `af::IoFixedFile::write_at()` / `fsync()` / `read_at()` 复用 fixed file table。
