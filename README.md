...
- `examples/io_uring_fixed_file.cpp`：使用 `async::io_register_files()` + `async::io_register_buffers()` + `af::IoFixedFile::write_fixed_at()` / `fsync()` / `read_fixed_at()` 组合 fixed file table 与 fixed buffer table。
