...
- `tests/runtime_io_tests.cpp`：IO 线程调度、epoll readiness 恢复、io_uring 文件、TCP accept/connect/stream、vectored stream/file/datagram 和 UDP datagram helper、read/write/TCP/UDP helper 与 adapter、重复 fd wait 拒绝、HUP/EOF、非法 fd、worker 误用降级，以及 StopImmediately 清理 pending IO wait。
