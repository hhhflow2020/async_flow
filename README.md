...
业务侧更推荐使用 `af::io_accept_some()` / `af::io_connect()` / `af::io_read_some()` / `af::io_write_some()` / `af::io_recv_some()` / `af::io_send_some()` / `af::io_recv_from_some()` / `af::io_send_to_some()` 这组 helper。普通 epoll 线程会先尝试一次非阻塞 syscall；遇到 `EAGAIN` / `EWOULDBLOCK` 时注册对应 readiness，并返回 `IoStep::Pending`。`ThreadKind::IoUring` 线程上，TCP `accept/connect/recv/send` 会优先提交 `IORING_OP_ACCEPT` / `IORING_OP_CONNECT` / `IORING_OP_RECV` / `IORING_OP_SEND`，UDP `recv_from_some()` / `send_to_some()` 会优先提交 `IORING_OP_RECVMSG` / `IORING_OP_SENDMSG`；当 backend 不可用、ring 满或 socket completion 返回 would-block 时退回 epoll readiness：
...
- `ThreadKind::IoUring` 在同一个 IO executor 内用 raw io_uring syscall 提交 `read_at` / `write_at` / `fsync`、TCP `accept/connect/recv/send` 和 UDP `recvmsg/sendmsg`，completion 通过 eventfd 唤醒，不把 IO completion 跨线程搬运。
...
- `tests/runtime_io_tests.cpp`：IO 线程调度、epoll readiness 恢复、io_uring 文件、TCP accept/connect/stream 和 UDP datagram helper、read/write/TCP/UDP helper 与 adapter、重复 fd wait 拒绝、HUP/EOF、非法 fd、worker 误用降级，以及 StopImmediately 清理 pending IO wait。
