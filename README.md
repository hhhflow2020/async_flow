...
业务侧更推荐使用 `af::io_accept_some()` / `af::io_connect()` / `af::io_read_some()` / `af::io_write_some()` / `af::io_recv_some()` / `af::io_send_some()` / `af::io_recv_from_some()` / `af::io_send_to_some()` 这组 helper。需要减少协议拼包拷贝时，可使用 `readv/writev`、`recvv/sendv`、`recvv_from/sendv_to` 和 `readv_at/writev_at` 这组 scatter/gather helper；需要内核态搬运时，可使用 `af::io_sendfile_some()` / `af::io_splice_some()`。普通 epoll 线程会先尝试一次非阻塞 syscall；遇到 `EAGAIN` / `EWOULDBLOCK` 时注册对应 readiness，并返回 `IoStep::Pending`。`ThreadKind::IoUring` 线程上，`io_openat()`、`io_close()`、`io_statx()`、`io_fallocate()`、`io_renameat()`、`io_unlinkat()`、`io_splice_some()` 和 TCP `accept/connect/recv/send` 会优先提交 `IORING_OP_OPENAT` / `IORING_OP_CLOSE` / `IORING_OP_STATX` / `IORING_OP_FALLOCATE` / `IORING_OP_RENAMEAT` / `IORING_OP_UNLINKAT` / `IORING_OP_SPLICE` / `IORING_OP_ACCEPT` / `IORING_OP_CONNECT` / `IORING_OP_RECV` / `IORING_OP_SEND`，stream/datagram vectored IO 会优先提交 `IORING_OP_RECVMSG` / `IORING_OP_SENDMSG`，文件 vectored IO 会优先提交 `IORING_OP_READV` / `IORING_OP_WRITEV`，UDP `recv_from_some()` / `send_to_some()` 会优先提交 `IORING_OP_RECVMSG` / `IORING_OP_SENDMSG`；当 backend 不可用、ring 满或 socket completion 返回 would-block 时退回 epoll readiness：
...
已经挂起的 epoll readiness IO 可在对应 IO 线程调用 `async::cancel_io(thread, state)` 取消；runtime 会从 epoll 删除 fd wait，并用 `ECANCELED` 恢复原 pending task。为了避免跨线程改动 `IoOpState` 和 executor 内部 wait 表，业务侧应把取消动作也调度到 fd 所属 IO 线程执行；io_uring completion 级取消和 per-operation timeout 是后续补齐项。
...
- `async::cancel_io(thread, state)` 可取消同 IO 线程内的 epoll readiness pending wait，并用 `ECANCELED` 恢复原 task；正常 helper 热路径只增加一个 `[[unlikely]]` 取消分支。
