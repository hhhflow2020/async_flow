...
- `af::IoFile` / `af::TcpListener` / `af::TcpStream` / `af::UdpSocket` / `af::IoTimer` 是零堆分配 adapter，仅保存 `thread + fd` 并内联转发到 helper；它们不会引入额外队列或 MPMC hop。
...
- `tests/runtime_io_tests.cpp`：IO 线程调度、epoll readiness 恢复、timerfd、io_uring 文件、TCP accept/connect/stream、vectored stream/file/datagram 和 UDP datagram helper、read/write/TCP/UDP helper 与 adapter、重复 fd wait 拒绝、HUP/EOF、非法 fd、worker 误用降级，以及 StopImmediately 清理 pending IO wait。
